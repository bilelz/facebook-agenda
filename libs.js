...
var divs = document.querySelectorAll('[data-timestamp]');
[].forEach.call(divs, function(div) {
    div.innerHTML = moment(parseInt(div.getAttribute("data-timestamp"),10)*1000).fromNow();
});

if(navigator.share !== undefined){
    document.body.classList.add("web-share-api");
    document.querySelector('[data-share-api]').addEventListener('click',function(event){
        navigator.share({
...
if ('serviceWorker' in navigator) {
    ...
    .then(function(reg){
        //console.log("Will the service worker register? Yes, it did.");
    }).catch(function(err) {
        console.log("Will the service worker register? No it didn't. This happened: ", err)
    });
}
...
window.addEventListener('beforeinstallprompt', function(e) {
    ...
    // si le addToHomescreen a dÃ©jÃ  Ã©tait prÃ©sentÃ©, on le cache
    if(localStorage.getItem("addToHomescreenAsked")){
        e.preventDefault();
        return false;
    }
    ...
    e.userChoice.then(function(choiceResult) {
        ...
        localStorage.setItem("addToHomescreenAsked","true");
